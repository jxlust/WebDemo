$(function () {
    ...
    var time_expire = 300;
    var pay_success = 3;
    var cz_channel = $("input[name='cz_channel']").val();
    var order_id =  $("input[name='order_id']").val();

    var loadPay = setInterval(function () {
        var url = '/pay/check_pay';
        $.post(url, {service: "ll.order.check", jkma: jkma, cz_channel: cz_channel}, function (res) {
                if (res.status == 500) {
                    layer.msg(res.msg);
                    ...
                    pay_success--;
                    if (client == '3DLLManager' || client == '3d66') {
                        $('.content').html(
                            "<div class='success'><i class='iconfont'>&#xe6a1;</i> <h2>恭喜您，充值成功</h2><p style='text-align: center'><span>" + pay_success + "</span>&nbsp;秒后关闭</p></div>"
                        );
                    } else {
                        $('.content').html(
                            "<div class='success'><i class='iconfont'>&#xe6a1;</i> <h2>恭喜您，充值成功</h2><p><span>" + pay_success + "</span>&nbsp;秒后跳到【个人中心 - 资产中心 - " + payname + "】</p></div>"
                        );
                    }
                    ...
                    if (pay_success == 0) {
                        ...
                        if (client == '3DLLManager') {
                            ...
                            } else if (payname.indexOf('商城') > -1) {
                                parent.window.location.href = objSite.getMallUrl('pay/'+order_id+'.html');
                            } else if (payname.indexOf('魔术手') > -1) {
    ...
    $(document).on('click', '#talkus', function () {
        ...
        if (client == '3DLLManager') {
            ...
            parent.ThirdPlatformWebView.openExternalLink(url);
            return false;
        } else {
